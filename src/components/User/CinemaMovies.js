...
function CinemaMovies(props) {
      ...
      cinema.danhSachPhim.forEach(movie => {
        if (
          movie.lstLichChieuTheoPhim.findIndex(
            movie => movie.ngayChieuGioChieu.slice(0, 10) === today
          ) > -1
        ) {
            ...
            >
              <div className="movie--small">
                <img
                  ...
                  src={
                    props.listMovie.find(
                      element => element.maPhim === movie.maPhim
                    )
                      ? props.listMovie.find(
                          element => element.maPhim === movie.maPhim
                        ).hinhAnh
                ...
                <div className="movie__info">
                  <span className="movie__name">{movie.tenPhim}</span>
                  <span>120 phút</span>
                </div>
              </div>
            </ExpansionPanelSummary>
            <ExpansionPanelDetails>
              <Typography>
                {movie.lstLichChieuTheoPhim
                  .filter(
                    showtime =>
                      showtime.ngayChieuGioChieu.slice(0, 10) === today
                  )
                  .map((showtime, index) => {
                    return (
                      <NavLink
  ...
  return (
    <div className="cinema-movies flex-grow-1" style={{ width: "min-content" }}>
      {renderShowtimes()}
    </div>
  );
}
