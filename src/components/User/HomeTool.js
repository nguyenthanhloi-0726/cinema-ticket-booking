...
import * as action from "../../services/redux/actions/homeToolActions";
          ...
          groupCinema.cumRapChieu.forEach(cinema => {
            if (cinema.tenCumRap === quickTicket.cinemaSelect) {
              showtimes = cinema.lichChieuPhim.filter(
                showtime => showtime.ngayChieuGioChieu.slice(0, 10) === value
              );
            }
          });
        });
      ...
      <div className="home-tool">
        <Paper elevation={3} className={classes.paper}>
          <FormControl className={classes.movie}>
            ...
            >
              {props.listMovie.map((movie, index) => (
                <MenuItem key={index} id={movie.maPhim} value={movie.tenPhim}>
                  {movie.tenPhim}
                </MenuItem>
              ))}
            </Select>
          ...
          <FormControl className={classes.showtime}>
            <InputLabel id="showtime-select-label">Suất chiếu</InputLabel>
...
const mapStateToProps = state => {
  return {
    ...
    showtimeInfo: state.homeToolReducer.showtimeInfo
  };
};
...
export default connect(mapStateToProps, mapDispathToProp)(HomeTool);
